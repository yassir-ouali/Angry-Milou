...
public class ChooseLevel extends Screen{
	...
	public void update(float deltaTime) {
		...
		for(int i=0;i<TouchEvents.size();i++){
			...
			if(event.type==TouchEvent.TOUCH_DOWN){
				...
				}else if(inBounds(event, 16*deltaWidth/3,9*deltaHeight/2,deltaWidth/2,deltaHeight*3/2)){
	...
	public void paint(float deltaTime) {
		...
		g.drawScaledImage(Assets.universes,deltaWidth*2,deltaHeight/2,deltaWidth*2,deltaHeight*2,0,0, 200,50);
		
		g.drawScaledImage(Assets.arrowRight,16*deltaWidth/3,9*deltaHeight/2,deltaWidth/2,deltaHeight*3/2,0,0, 158, 240);
